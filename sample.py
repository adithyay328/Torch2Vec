...
# of training tensors: (OneHotInputTensorsInWindow, [OneHotOutputTensor])
class DocumentSampler(torch.utils.data.IterableDataset):
    ...
    def __init__(self, listOfDocs, vectorizer, windowSize, skipgram):
        ...
        for docNum in range(len(vectorizedDocs)):
            for tokIndex in range(len(vectorizedDocs[docNum])):
                ...
                # Literally the only difference between skipgram and CBOW
                # is the order of this following line
                if skipgram:
                    ...
                    self.listOfSampleTuples.append(
                        ( listOfWindowTensors, currentTokenTensor )
                    )
